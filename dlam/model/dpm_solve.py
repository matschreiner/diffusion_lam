...
class NoiseScheduleVP:  # pragma: no cover
    def __init__(
        self,
        schedule="discrete",
        betas=None,
...
def model_wrapper(  # pragma: no cover
    model,
    noise_schedule,
    model_type="noise",
    model_kwargs={},
...
class DPM_Solver:  # pragma: no cover
    def __init__(
        self,
        model_fn,
        noise_schedule,
    ...
    def sample(
        ...
        with torch.no_grad():
            if method == "adaptive":
                ...
                for step in tqdm(
                    range(order, steps + 1), leave=False, desc="ODE Sample"
                ):
                    t = timesteps[step]
...
def interpolate_fn(x, xp, yp):  # pragma: no cover
    """
...
def expand_dims(v, dims):  # pragma: no cover
    """
